...
const ImplV3 = artifacts.require("USDPImplementationV3.sol");
...
describe("UpgradeToARD", function ([
  _,
  admin,
  owner,
  supplyController,
  ...
  beforeEach(async function () {
    ...
    this.setData = async function () {
      ...
      await this.token.transfer(supplyController, subAmount, {
        from: purchaser,
      });
    ...
    this.upgrade = async function () {
      ...
      await this.proxy.upgradeToAndCall(this.implV3.address, funcSig, {
        from: admin,
      });
    };
    ...
    this.checkData = async function () {
      ...
      assert.strictEqual(
        this.currentAP,
        await this.newToken.assetProtectionRole()
      );
      assert.deepStrictEqual(
        this.sCBalance,
        await this.token.balanceOf(supplyController)
      );
      assert.deepStrictEqual(
        this.purchaserBalance,
      ...
      assert.deepStrictEqual(
        this.bystanderApproval,
        await this.token.allowance(holder, bystander)
      );
      assert.deepStrictEqual(
        this.frozenApproval,
        await this.token.allowance(holder, frozen)
      ); // 0
      assert.strictEqual(
        this.bystanderFrozen,
        await this.token.isFrozen(bystander)
      );
