...
describe("ARD Protection Functionalities", function () {
  ...
  describe("when the asset protection role is unset", function () {
    it("reverts asset protection actions", async function () {
      ...
      await expect(this.token.wipeFrozenAddress(freezableAddress)).to.be.reverted;
    });
  });
  ...
  describe("as an asset protectable token", function () {
    ...
    describe("after setting the AssetProtectionRole", function () {
      it("the current asset protection role is set", async function () {
        const currentAssetProtectionRole = await this.token.isAssetProtection(this.protector.address);
    ...
    describe("freeze", function () {
      ...
      it("adds the frozen address", async function () {
        ...
        const isFrozen = await this.token.connect(this.protector).isFrozen(this.user1.address);
        assert.equal(isFrozen, true, "address is frozen");
      });

      it("emits an AddressFrozen event", async function () {
        await expect(this.token.connect(this.protector).freeze(this.user1.address))
      ...
      describe("when frozen", function () {
        ...
        beforeEach(async function () {
          ...
          await this.token.connect(this.user1).approve(this.user2.address, approvalAmount);

          // approve freezableAddress address to take some of those tokens from otherAddress
          await this.token.connect(this.user2).approve(this.user1.address, approvalAmount);
        ...
        it("reverts when transfer is from frozen address", async function () {
          await expect(this.token.connect(this.user1).transfer(this.user2.address, amount)).to.be.reverted;
        });

        it("reverts when transfer is to frozen address", async function () {
          await expect(this.token.connect(this.user2).transfer(this.user1.address, amount)).to.be.reverted;
        });

        it("reverts when transferFrom is by frozen address", async function () {
          await expect(this.token.connect(this.user1).transferFrom(this.user2.address,this.user2.address, approvalAmount)).to.be.reverted;
        });

        it("reverts when transferFrom is from frozen address", async function () {
          await expect(this.token.connect(this.user2).transferFrom(this.user1.address,this.user2.address, approvalAmount)).to.be.reverted;
        });

        it("reverts when transferFrom is to frozen address", async function () {
          await expect(this.token.connect(this.user2).transferFrom(this.user2.address,this.user1.address, approvalAmount)).to.be.reverted;
        });
        ...
        it("reverts when approve spender is the frozen address", async function () {
          await expect(this.token.connect(this.user2).approve(this.user1.address,approvalAmount)).to.be.reverted;
        });
      });
      ...
      it("reverts when address is already frozen", async function () {
        ...
        await this.token.connect(this.protector).freeze(this.user1.address);
        await expect(this.token.connect(this.protector).freeze(this.user1.address)).to.be.reverted;
      });
    });

    describe("unfreeze", function () {
      it("reverts when address is already unfrozen", async function () {
        await expect(this.token.connect(this.protector).unfreeze(this.user1.address)).to.be.reverted;
      });
      ...
      describe("when already frozen", function () {
        ...
        it("reverts when sender is not asset protection", async function () {
          await expect(this.token.connect(this.user2).unfreeze(this.user1.address)).to.be.reverted;
        });
        ...
        it("unfrozen address can transfer again", async function () {
          ...
          await this.token.connect(this.supplyController).transfer(this.user1.address, amount);

          let balance = await this.token.balanceOf(this.user1.address);
          assert.equal(amount, balance);

          await this.token.connect(this.user1).transfer(this.owner.address, amount);
        ...
        it("emits an AddressFrozen event", async function () {
          await expect(this.token.connect(this.protector).unfreeze(this.user1.address))
    ...
    describe("wipeFrozenAddress", function () {
      it("reverts when address is not frozen", async function () {
        await expect(this.token.connect(this.protector).wipeFrozenAddress(this.user1.address)).to.be.reverted;
      });
      ...
      describe("when already frozen with assets and approvals", function () {
        ...
        it("reverts when sender is not asset protection", async function () {
          await expect(this.token.connect(this.user2).wipeFrozenAddress(this.user1.address)).to.be.reverted;
        });

        it("wipes a frozen address balance", async function () {
          await this.token.connect(this.protector).wipeFrozenAddress(this.user1.address);
        ...
        it("emits an FrozenAddressWiped event", async function () {
          await expect(this.token.connect(this.protector).wipeFrozenAddress(this.user1.address))
            .to.emit(this.token, "FrozenAddressWiped").withArgs(this.user1.address)
            .to.emit(this.token, "SupplyDecreased").withArgs(this.user1.address,amount)
            .to.emit(this.token, "Transfer").withArgs(this.user1.address,ZERO_ADDRESS,amount);
        });
      });
    });
    ...
    describe("setAssetProtectionRole", function () {
      it("reverts if sender is not owner or AssetProtectionRole", async function () {
        await expect(this.token.connect(this.user1).setAssetProtectionRole(this.user1.address)).to.be.reverted;
      });
      ...
      it("works if sender is AssetProtectionRole", async function () {
        ...
        const currentAssetProtectionRole = await this.token.isAssetProtection(this.protector.address);
        assert.equal(currentAssetProtectionRole, true);
        const notAssetProtectionRole = await this.token.isAssetProtection(this.user1.address);
      ...
      it("emits a AssetProtectionRoleSet event", async function () {
        await expect(this.token.setAssetProtectionRole(this.user1.address))
          ...
          .withArgs(this.token.ASSET_PROTECTION_ROLE,this.user1.address,this.owner.address);
      });
    });
  });
});
