...
describe("Pausable ARD", function () {
  ...
  it("cannot grant roles in pause", async function () {
    ...
    await expect(this.token.connect(this.owner).setMinterRole(this.user1.address)).to.be.reverted;
    await expect(this.token.connect(this.owner).setBurnerRole(this.user1.address)).to.be.reverted;
    await expect(this.token.connect(this.owner).setAssetProtectionRole(this.user1.address)).to.be.reverted;
    await expect(this.token.connect(this.owner).setSupplyControllerRole(this.user1.address)).to.be.reverted;
  });

  it("cannot transfer in pause", async function () {
    // pause the contract and checks to emit a Pause event
    await expect(this.token.pause()).to.emit(this.token, "Paused").withArgs(this.owner.address);
    const paused = await this.token.paused();

    assert.equal(paused, true);
    await expect(this.token.connect(this.owner).transfer(this.user1.address, amount)).to.be.reverted;
    const balance = await this.token.balanceOf(this.owner.address);
  ...
  it("cannot approve/transferFrom in pause", async function () {
    ...
    await this.token.pause();
    await expect(this.token.connect(this.owner).approve(this.user2.address, 2 * amount)).to.be.reverted;
    await expect(this.token.connect(this.user2).transferFrom(this.owner.address,this.user2.address, amount)).to.be.reverted;
  });

  it("should resume allowing normal process after pause is over", async function () {
    await this.token.pause();
    // unpause the contract and checks to emit a Unpause event
    await expect(this.token.unpause()).to.emit(this.token, "Unpaused").withArgs(this.owner.address);
    await this.token.transfer(this.user2.address, amount);
    const balance = await this.token.balanceOf(this.owner.address);
    assert.equal(90, balance);
  });
